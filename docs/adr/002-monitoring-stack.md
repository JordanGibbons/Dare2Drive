...
4. **Can the team be paged to their phones when something goes wrong?**
...
| **Alerting** | **Prometheus Alertmanager** + custom **ntfy-relay** | Alertmanager evaluates rules; ntfy-relay translates webhooks → **ntfy.sh** phone push notifications |
...
```
Prometheus ─── fires alert ──▶ Alertmanager ──▶ ntfy-relay:9096/ntfy
                                                       │
...
1. Set `NTFY_TOPIC` in `.env` to a long, unguessable string (e.g. `dare2drive-xK9mP3Qq8r`).
2. Team members install the **ntfy** app and subscribe to that topic.
3. Done — no accounts, no credit card, completely free.
...
The alerting/ntfy path stays the same regardless.
...
- New env vars: `NTFY_TOPIC`, `NTFY_URL`, `NTFY_TOKEN`, `GRAFANA_ADMIN_USER`, `GRAFANA_ADMIN_PASSWORD` — all documented in `.env.example`.
...
cp .env.example .env
# Set NTFY_TOPIC to something private, e.g. dare2drive-$(openssl rand -hex 8)
