...
class HullClassSelect(discord.ui.View):
    ...
    async def _handle_choice(self, interaction: discord.Interaction, hull_class: HullClass) -> None:
        ...
        async with async_session() as session:
            ...
            # Create default empty build with an autogenerated ship name —
            # players can rename later via /ship rename. Empty-build aggregate
            # stats default to 0s, which is fine for the namer's pool lookup.
            starter_name = generate_ship_name(RaceFormat.SPRINT, hull_class, None)
            build = Build(
